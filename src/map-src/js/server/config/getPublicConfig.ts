...
app.post('/getPublicConfig', async (c) => {
  c.header('Content-Type', 'application/json; charset=utf-8');
  try {
    const body = await c.req.json();
    console.log('getPublicConfig実行:', body);
    ...
    for (let i = 0; i < getConfigs.rows.length; i++) {
      ...
      config['config' + (i + 1)]['opup_row_num'] = getPopups.rows.length;
    ...
    console.error('getPublicConfigエラー:', error);
    return c.json({
      success: false,
