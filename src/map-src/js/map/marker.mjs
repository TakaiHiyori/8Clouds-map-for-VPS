 ...
 * 作成日：2025/11/10
 */
export const marker = async (record, config, domain, login, latLngBox, map) => {
 ...
 * 作成日：2025/11/10
 */
export const checkRecord = (record, config) => {
    ...
    for (let i = 1; i <= config.narrow_row_number; i++) {
        ...
        switch (record[condition.field].type) {
            ...
            case 'CREATED_TIME':
            case 'UPDATED_TIME':
                const datetime = luxon.DateTime.fromISO(record[condition.field].value);
                const conditionDatetime = luxon.DateTime.fromISO(conditionValue);
