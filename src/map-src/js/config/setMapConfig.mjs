export const setConfigName = async (config, checkLogin) => {
    ...
    for (const key in config) {
        if (!config[key].mapTitle) {
            ...
            $('#' + key + ' #valid' + num).attr('checked', config[key].valid).prop('checked', config[key].valid);
            if (!config[key].valid) {
            ...
            if (config[key].openURL !== '') {
                $('#' + key + ' .check-open-map').attr({ 'href': `https://solution.8clouds.co.jp/benri/${config.openDomain}/${config[key].openURL}`, 'target': '_blank' }).show();
            }
